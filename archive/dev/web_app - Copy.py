...
# Hallway
@app.route("/bulkhead/<int:state>", methods=['POST'])
def bulkhead(state):
    if state == 0:
        pi_thing.set_bulkhead(True)
    elif state == 1:
        pi_thing.set_bulkhead(False)
    else:
        return ('Unknown LED state', 400)
    return ('', 204)

@app.route("/hallway/<int:state>", methods=['POST'])
def hallway(state):
    if state == 0:
        pi_thing.set_hallway(True)
    elif state == 1:
        pi_thing.set_hallway(False)
    else:
        return ('Unknown LED state', 400)
    return ('', 204)   
...
def bedroom_hair_on(): 
    jarvis_control.bedroom_hair_on()

@app.route("/bedroom_hair_off", methods=['POST'])
def bedroom_hair_off(): 
    jarvis_control.bedroom_hair_off()
...
# Bathroom
    'Bathroom Light reading':BathroomLightReading,
    'Bathroom bathroom Light':BathroomLightStatus,
    'Bathroom Light commanded':BathroomLightCommanded,
    'Bathroom Temperature':BathroomTemperature,
    'Bathroom Humidty':BathroomHumidity,
